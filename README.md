...
**The Problem**: When processing datasets with numerous nodes or strong correlations, LiM tends to generate a dense initial graph. This causes an exponential () combinatorial explosion during the local search phase (when attempting to reverse edges), which easily leads to Out-Of-Memory (OOM) errors or prolonged program freezing.

**The Solution**: To ensure the algorithm's usability under limited computational resources, we introduced hard truncation strategies:
