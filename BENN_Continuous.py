...
def direction_belt_1d(
    ...
    Yc = norm(Y) if Y_type == "cont" else Y.astype(int)
    n = len(Xc)
    ...
    sigma_Y = max(std_Y, 1e-8)
    lo_X, hi_X = mu_X - 2.0 * sigma_X, mu_X + 2.0 * sigma_X
    lo_Y, hi_Y = mu_Y - 2.0 * sigma_Y, mu_Y + 2.0 * sigma_Y
    m = hp1['m_output_dim']
    X_centers = rng.uniform(lo_X, hi_X, size=(1, m))
    Y_centers = rng.uniform(lo_Y, hi_Y, size=(1, m))

    # ===========================================================
    # A : X -> Y
    # ===========================================================
    Xa_tr, Xa_te, Ya_tr, Ya_te = train_test_split(Xfea, Yc, test_size=test_size, random_state=seed)
    ...
    model_A = train_model(
        ...
        warmup_epochs=hp2['warmup_epochs'], centers =  Y_centers,sigma = sigma_Y,
        device=device
    )

    zA_tr = belt_embed(model_A, Xa_tr)
    zA_va = belt_embed(model_A, Xa_va)
    zA_te = belt_embed(model_A, Xa_te)

    nnA = train_tiny_regressor(zA_tr, Ya_tr, zA_va, Ya_va, n)
    with torch.no_grad():
        rA1 = (to_t(Ya_te.reshape(-1, 1)) - nnA(to_t(zA_te))).cpu().numpy()

    zA_rf = np.vstack([zA_tr, zA_va])
    yA_rf = np.concatenate([Ya_tr, Ya_va], axis=0).ravel()
    rfA = RandomForestRegressor(n_estimators=200, min_samples_leaf=6, n_jobs=-1, random_state=seed)
    rfA.fit(zA_rf, yA_rf)
    rA2 = Ya_te.reshape(-1, 1) - rfA.predict(zA_te).reshape(-1, 1)

    K_A = rbf_kernel(Xa_te.reshape(-1, 1))
    stat_A1, p_A1 = hsic_test(K_A, rbf_kernel(rA1))
    stat_A2, p_A2 = hsic_test(K_A, rbf_kernel(rA2))

    # ===========================================================
    # B : Y -> X
    # ===========================================================
    Xb_tr, Xb_te, Yb_tr, Yb_te = train_test_split(Yfea, Xc, test_size=test_size, random_state=seed)
    ...
    K_B = rbf_kernel(Xb_te.reshape(-1, 1))
    ...
    decision_rf = decide(p_A2, p_B2, stat_A2, stat_B2)
