...
indtestHsic <- function(X,Y,alpha=alpha,pars,kernel=F)
{
  ...
  if (kernel){
    if (n > 1000) {
      return(dhsic.test(X, Y, kernel = c("discrete", "gaussian"), method = "gamma"))
    } else {
      return(dhsic.test(X, Y, kernel = c("discrete", "gaussian"), method = "bootstrap", B = 1000))
    }
    
  } else {
    if (n > 1000) {
      return(dhsic.test(Y, X, method = "gamma"))
    } else {
