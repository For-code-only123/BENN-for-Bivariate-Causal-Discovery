...
def direction_belt_1d(
    ...
    true_idx = np.argmax(Ya_te, axis=1)                            # shape: (n_te,)
    rA1 =  true_idx - pred_idx

    K_A = delta_kernel(np.argmax(Xa_te, axis=1))
    stat_A1, p_A1 = hsic_test(K_A, delta_kernel(rA1))
    ...
    with torch.no_grad():
        ...
        probs_np = probs.cpu().numpy()
        pred_idx = np.argmax(probs_np, axis=1)
    true_idx = np.argmax(Yb_te, axis=1)
    rB1 = true_idx - pred_idx

    K_B = delta_kernel(np.argmax(Xb_te, axis=1))
    stat_B1, p_B1 = hsic_test(K_B, delta_kernel(rB1))
